...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            # Read image
            self.count += 1
            im0 = cv2.imread(path,cv2.IMREAD_UNCHANGED)  # BGR #KENNEDYMOTA
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        print("On LoadImagesAndLabels ",self.img_size)
    ...
    def check_cache_ram(self, safety_margin=0.1, prefix=""):
        ...
        for _ in range(n):
            im = cv2.imread(random.choice(self.im_files), cv2.IMREAD_UNCHANGED)  # sample image #KENNEDYMOTA
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            # Albumentations
            #print(img.shape)
            imgx, labels = self.albumentations(img[:,:,0:3], labels)
    ...
    def load_image(self, i):
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                im = cv2.imread(f,cv2.IMREAD_UNCHANGED)  # BGR #KENNEDYMOTA
    ...
    def cache_images_to_disk(self, i):
        ...
        f = self.npy_files[i]
        if not f.exists():
            np.save(f.as_posix(), cv2.imread(self.im_files[i]), cv2.IMREAD_UNCHANGED) #KENNEDYMOTA
...
def extract_boxes(path=DATASETS_DIR / "coco128"):
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in IMG_FORMATS:
            # image
            im = cv2.imread(str(im_file), cv2.IMREAD_UNCHANGED)[..., ::-1]  # BGR to RGB #KENNEDYMOTA
            h, w = im.shape[:2]
...
class HUBDatasetStats:
    ...
    def _hub_ops(self, f, max_dim=1920):
        ...
        try:  # use PIL
            ...
            im = cv2.imread(f, cv2.IMREAD_UNCHANGED) #KENNEDYMOTA
...
class ClassificationDataset(torchvision.datasets.ImageFolder):
    ...
    def __getitem__(self, i):
        ...
        if self.cache_ram and im is None:
            im = self.samples[i][3] = cv2.imread(f, cv2.IMREAD_UNCHANGED) #KENNEDYMOTA
        elif self.cache_disk:
            if not fn.exists():  # load npy
                np.save(fn.as_posix(), cv2.imread(f, cv2.IMREAD_UNCHANGED)) #KENNEDYMOTA
            im = np.load(fn)
        else:  # read image
            im = cv2.imread(f, cv2.IMREAD_UNCHANGED)  # BGR  #KENNEDYMOTA
